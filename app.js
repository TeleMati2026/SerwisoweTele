...
function showTab(name){document.querySelectorAll('.app-tab').forEach(x=>x.classList.toggle('active',x.id===`tab-${name}`));
...
document.querySelectorAll('.nav-btn').forEach(b=>b.onclick=()=>{showTab(b.dataset.tab);if(b.dataset.tab==='platform')loadPlatformCompanies()});
...
$('showLogin').onclick=()=>openAuth('login');$('showRegister').onclick=()=>openAuth('register');$('switchMode').onclick=()=>setMode(mode==='login'?'register':'login');$('closeDialog').onclick=()=>$('authDialog').close();
