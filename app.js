...
let mode='login',demoMode=false,currentOrg=null,currentMember=null;
let customers=[],orders=[],prices=[],locations=[],orderHistory=[],selectedOrderId=null,inventoryProducts=[],inventoryMovements=[],orderParts=[],invoiceLines=[];
...
function setMode(next){mode=next;const reg=mode==='register';$('registerFields').classList.toggle('hidden',!reg);$('formEyebrow').textContent=reg?'Utwórz swój serwis':'Witaj ponownie';$('formTitle').textContent=reg?'Załóż firmę':'Logowanie';$('submitAuth').textContent=reg?'Utwórz konto i firmę':'Zaloguj się';$('switchMode').textContent=reg?'Masz już konto? Zaloguj się':'Nie masz konta? Załóż serwis';$('password').autocomplete=reg?'new-password':'current-password';$('authMessage').textContent=''}
...
function showTab(name){document.querySelectorAll('.app-tab').forEach(x=>x.classList.toggle('active',x.id===`tab-${name}`));document.querySelectorAll('.nav-btn').forEach(x=>x.classList.toggle('active',x.dataset.tab===name))}
...
async function loadCompany(){
  ...
  const {data:members,error}=await db.from('organization_members').select('organization_id,role,display_name,organizations(id,name,status,trial_ends_at)').eq('user_id',session.user.id).eq('active',true).limit(1);
  if(error){console.error(error);showLanding();return}
  const m=members?.[0];
  if(!m){showLanding();$('authMessage').textContent='Konto nie jest przypisane do firmy.';return}
  currentMember=m;currentOrg=m.organizations;
  $('companyTitle').textContent=currentOrg?.name||'Twoja firma';$('trialBadge').textContent=currentOrg?.status==='trial'?'Okres próbny':'Aktywne';
  $('demoBadge').classList.add('hidden');$('panelEyebrow').textContent='Panel właściciela';
  showDashboard();await loadAll();
}
async function loadAll(){
  const oid=currentOrg?.id;if(!oid)return;
  const [c,o,p,l,s,h,ip,im,op]=await Promise.all([
  ...
  [c,o,p,l,h,ip,im,op].forEach(r=>{if(r.error)console.error(r.error)});
  customers=c.data||[];orders=o.data||[];prices=p.data||[];locations=l.data||[];orderHistory=h.data||[];inventoryProducts=ip.data||[];inventoryMovements=im.data||[];orderParts=op.data||[];
...
function renderAll(){renderMetrics();renderOrders();renderCustomers();renderPrices();renderInventory();renderLocations();renderStaff();populateOrderSelects()}
function renderMetrics(){
  $('metricOpenOrders').textContent=orders.filter(x=>!['Wydane'].includes(x.status)).length;
  $('metricReadyOrders').textContent=orders.filter(x=>x.status==='Gotowe do odbioru').length;
  $('metricRevenue').textContent=money(orders.reduce((a,x)=>a+Number(x.estimated_amount||0),0));
  $('welcomeTitle').textContent=demoMode?'Witaj w wersji demonstracyjnej 👋':'SerwisoweTele działa 🎉';
  $('welcomeText').textContent=demoMode?'Możesz swobodnie przeglądać przykładowe zlecenia, klientów, cennik i lokalizacje.':'Zacznij od dodania pierwszego klienta, lokalizacji i zlecenia.';
  $('recentOrders').innerHTML=orders.slice(0,5).map(o=>`<div class="status-summary"><div><span>${esc(o.order_number)} · ${esc(o.brand)} ${esc(o.model)}</span><b>${esc(o.status)}</b></div></div>`).join('')||'<p class="empty-row">Brak zleceń.</p>';
...
function renderStaff(){const demo=demoMode?[{name:'Kamil Nowak',role:'Serwisant',orders:38},{name:'Anna Wiśniewska',role:'Pracownik',orders:51},{name:'Marek Zieliński',role:'Administrator',orders:29}]:[{name:currentMember?.display_name||'Właściciel firmy',role:'Właściciel',orders:orders.length}];$('staffGrid').innerHTML=demo.map(s=>`<article class="staff-card"><span class="card-badge">${esc(s.role)}</span><h3>${esc(s.name)}</h3><p>Obsłużone zlecenia: <b>${s.orders}</b></p></article>`).join('')}
function populateOrderSelects(){$('orderCustomer').innerHTML=customers.map(c=>`<option value="${c.id}">${esc(c.full_name)} — ${esc(c.phone)}</option>`).join('');$('orderLocation').innerHTML='<option value="">Bez przypisania</option>'+locations.map(l=>`<option value="${l.id}">${esc(l.name)}</option>`).join('')}
...
$('orderForm').onsubmit=async e=>{e.preventDefault();if(guardDemo())return;if(!customers.length)return $('orderError').textContent='Najpierw dodaj klienta.';const id=$('orderId').value;let orderNumber=null;if(!id){const {data,error}=await db.rpc('next_order_number');if(error)return $('orderError').textContent=error.message;orderNumber=data}const p={organization_id:currentOrg.id,customer_id:$('orderCustomer').value,location_id:$('orderLocation').value||null,brand:$('orderBrand').value.trim(),model:$('orderModel').value.trim(),imei:$('orderImei').value.trim()||null,issue_description:$('orderIssue').value.trim(),device_condition:$('orderCondition').value.trim()||null,internal_notes:$('orderInternalNotes').value.trim()||null,status:$('orderStatus').value,estimated_amount:Number($('orderAmount').value||0),due_date:$('orderDueDate').value||null,warranty_months:Number($('orderWarranty').value||0),accepted_date:$('orderAcceptedDate').value||new Date().toISOString().slice(0,10)};if(!id)p.order_number=orderNumber;const {error}=id?await db.from('service_orders').update(p).eq('id',id).eq('organization_id',currentOrg.id):await db.from('service_orders').insert(p);if(error)return $('orderError').textContent=error.message;$('orderDialog').close();e.target.reset();await loadAll()}
...
$('authForm').addEventListener('submit',async e=>{e.preventDefault();$('authMessage').textContent='Proszę czekać…';$('submitAuth').disabled=true;try{const email=$('email').value.trim(),password=$('password').value;if(mode==='login'){const {error}=await db.auth.signInWithPassword({email,password});if(error)throw error;$('authDialog').close();await loadCompany()}else{const displayName=$('displayName').value.trim(),companyName=$('companyName').value.trim(),companySlug=$('companySlug').value.trim();if(!displayName||!companyName||!companySlug)throw new Error('Uzupełnij dane właściciela i firmy.');const {data,error}=await db.auth.signUp({email,password,options:{data:{display_name:displayName}}});if(error)throw error;if(!data.session){$('authMessage').textContent='Sprawdź pocztę i potwierdź adres e-mail.';return}const {error:rpcError}=await db.rpc('create_my_organization',{company_name:companyName,company_slug:companySlug,owner_display_name:displayName});if(rpcError)throw rpcError;$('authDialog').close();await loadCompany()}}catch(err){$('authMessage').textContent=err.message||'Wystąpił błąd.'}finally{$('submitAuth').disabled=false}})
...
db.auth.onAuthStateChange(()=>setTimeout(loadCompany,0));loadCompany();
